...
void MYRCC_DeInit(void)
{
  ...
  RCC->CR &= (uint32_t)0xEAF6FFFF; // Reset HSEON, CSSON, PLLON, PLLI2S and PLLSAI(STM32F42/43xxx devices) bits
  RCC->PLLCFGR = 0x24003010; // Reset PLLCFGR register
  RCC->PLLI2SCFGR = 0x20003000; // Reset PLLI2SCFGR register
  RCC->PLLSAICFGR = 0x24003000; // Reset PLLSAICFGR register, only available for STM32F42/43xxx devices
  RCC->CR &= (uint32_t)0xFFFBFFFF; // Reset HSEBYP bit
  RCC->CIR = 0x00000000; // Disable all interrupts
  RCC->DCKCFGR = 0x00000000; // Disable Timers clock prescalers selection, only available for STM32F42/43xxx devices
}
//GPIO复用设置
//GPIOx:GPIOA~GPIOI.
//BITx:0~15,代表IO引脚编号.
